...
If you have to import the same set of modules everywhere, it can get tedious. Unlike Nest, [Angular](https://angular.io) `providers` are registered in the global scope. Once defined, they're available everywhere. Nest, however, encapsulates providers inside the module scope. You aren't able to use a module's providers elsewhere without first importing the encapsulating modules.

When you want to provide a set of providers which should be available everywhere out-of-the-box, (e.g., helpers, database connections, etc.) you can make the module **global** with the `@Global()` decorator.
